//! Level 0 discovery: the bounded breadth-first traversal that streams
//! generation-scoped rows, and the entry classification it applies.
...
use crfty_core::{
    ...
    AnalysisFileScan, AnalysisGenerationId, AnalysisRow, AnalysisRowEntry, AnalysisRowId, Command,
...
pub(super) struct DiscoveryRequest {
    ...
    pub(super) extensions: BTreeSet<VideoExtension>,
    pub(super) registry: Arc<Mutex<AnalysisGenerationRegistry>>,
    pub(super) cancellation: Cancellation,
}

pub(super) fn run_discovery_request(request: &DiscoveryRequest, commands: &CommandSender) {
...
pub(super) fn display_text(value: &OsStr) -> AnalysisDisplayText {
    AnalysisDisplayText {
...
pub(super) fn deduplicate_roots(roots: Vec<PathBuf>) -> Vec<PathBuf> {
