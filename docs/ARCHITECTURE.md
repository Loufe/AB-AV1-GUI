...
Located tools are only pending. Each session start runs a bounded, cancellable capability probe before the first claim: ffprobe's JSON version document, a one-second synthetic `libsvtav1` encode, and a `libvmaf` comparison of two synthetic inputs, each judged by exit status. The probe then queries FFmpeg for every hardware decoder CRFty can name. The verified revisions and the available decoders are the tool facts the reducer composes into every claim's execution (`compose_execution`). A changed toolchain therefore invalidates analysis reuse (ADR-007), and hardware decode is selected from probe evidence rather than by a lazy engine lookup. A failed probe ends the session with a typed reason and no reserved item; missing tools reject session and Basic Scan starts while the queue, history, statistics, and settings stay usable. The Settings view shows the typed failures with their paths and per-platform install guidance.
