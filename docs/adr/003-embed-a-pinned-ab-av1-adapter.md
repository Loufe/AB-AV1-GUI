...
Related: ADR-005 (unsafe policy) and ADR-023 (the user-supplied FFmpeg binaries this adapter drives). The job runtime, cancellation contract, and output-promotion transaction are in `docs/design/lifecycle.md`; the hazards this boundary carries into the port are in `docs/design/porting-hazards.md`.
