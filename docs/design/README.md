...
- `docs/*.md`: settled behaviour contracts, stating verified behaviour only.
...
A working note is material for a decision in progress. When its subject settles, distill it: decisions become ADRs, settled behaviour becomes a contract doc under `docs/`, and the note is deleted rather than left as a second unmaintained account of a subject covered elsewhere.
...
A document whose value does not expire, such as a survey of prior art or a record of an external tool's observed behaviour, is not working material. Mark it `Status: stable reference` and it persists.
