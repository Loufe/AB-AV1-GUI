...
**Measured.** Nineteen attempts were produced across the four searches. Four of them, one per search, correspond to the `Done` update that V3 retains today. The other fifteen are the observations the adapter drops.
...
**Measured.** Log lines are a worse source than the structured stream for the same run. Parsing ab-av1's stderr recovered boundaries for only 16 of 19 attempts: 4 of 5, 3 of 3, 6 of 6, and 3 of 5 by fixture. It recovered no per-sample score lines, while structured output carried every attempt in all four runs. This supports consuming the adapter's typed updates rather than adding a log parser.
...
**Derived.** Cost scales with pixel count rather than window length. Between 1080p and 2160p cuts of identical content and frame count, the window filter pass rose from 4.818 s to 18.792 s: a factor of 3.90 for four times the pixels. The packet pass rose by a factor of 2.05, and the search rose by a factor of 3.73.
...
**Measured.** Crop detection found the same 8-pixel letterbox on all three 1080p cuts, which is the correction a letterbox-aware bits-per-pixel figure needs and which no baseline predictor applies.
...
**Measured, one pair.** The same content encoded at two resolutions produced best CRF 35.25 at 1080p and 45.25 at 2160p, with predicted encode percents of 69.4 and 44.2. A CRF learned at one resolution does not transfer to another, even for identical content. This agrees with the current estimator's resolution bucketing.
...
**Measured.** Twenty fixtures were prepared and four were collected. The sixteen outstanding fixtures carry the content variety the question depends on. They include both remaining 4K segments, four segments each from two other films, three heavy-motion re-encodes of pristine masters, two controlled-grain variants, and one low-headroom recompressed source.
