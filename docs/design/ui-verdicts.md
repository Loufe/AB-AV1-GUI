...
- **D2, theme ownership**: the light/dark elevation tier is frontend-only state; the backend and journal store none of it. The tier is the single source of truth: shadcn component vocabulary resolves against it rather than holding its own colours.
- **D3, design tokens**: raw values live in `:root`/`.dark` as CSS custom properties and `@theme inline` references them, because literal colours in a plain `@theme` would be inlined at build time and break runtime theme switching.
