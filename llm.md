...
-   **convert_app/**: The main package containing all application modules.
    ...
    -   **ab_av1_wrapper.py**: Wrapper class (`AbAv1Wrapper`) for executing and managing the `ab-av1.exe` command-line tool, including output parsing, progress monitoring, error handling, and VMAF fallback logic.
...
-   **requirements.txt**: Lists Python package dependencies (though currently might be empty if only standard library + Tkinter is used).
...
-   External Tools:
    -   `ab-av1.exe`: Required for the core VMAF-targeted AV1 encoding. Assumed to be in the `convert_app` directory.
...
-   **VMAF-based Quality Targeting:** Leverages `ab-av1 auto-encode`'s ability to find the optimal CRF value that meets a specified VMAF target (`DEFAULT_VMAF_TARGET` in `utils.py`). Includes a fallback loop (`AbAv1Wrapper.auto_encode`) that lowers the target VMAF if the initial target is unattainable, down to a minimum (`MIN_VMAF_FALLBACK_TARGET`).
...
-   **Smart Skipping:** `conversion_controller.scan_video_needs_conversion` checks the output path (considering `overwrite` setting) and uses `get_video_info` to check if the input is already an AV1 video in an MKV container.
-   **Progress Tracking:** `AbAv1Wrapper._update_stats_from_line` parses `ab-av1`/`ffmpeg` output using regex to extract VMAF/CRF during search and percentage/FPS/ETA/size during encoding. These stats are passed via callbacks to `conversion_controller.handle_progress`, which uses `gui_updates` functions to update the UI. ETA and estimated size are calculated within `gui_updates.update_conversion_statistics`.
...
### Integration Method (`ab_av1_wrapper.py`)
...
-   `auto_encode`: Main method executing `ab-av1 auto-encode`.
    ...
    -   Calls `_update_stats_from_line` to parse each line.
    ...
    -   Calls `_parse_final_output` to extract stats missed during stream parsing.
    -   Renames temp file to final output on success.
    -   Calls `clean_ab_av1_temp_folders`.
-   `_update_stats_from_line`: Uses regex to parse VMAF, CRF, progress %, size %, FPS, ETA from `ab-av1` and `ffmpeg` output lines. Manages internal state (`phase`, `progress_quality`, `progress_encoding`). Triggers `file_info_callback` with structured progress data.
-   `_parse_final_output`: Fallback parsing of the full output text if stats weren't captured reliably during streaming.
-   `clean_ab_av1_temp_folders`: Utility to remove `.ab-av1-*` temporary directories.
-   `check_ab_av1_available`: Simple check for executable existence.
...
4.  **AB-AV1 Execution (`ab_av1_wrapper.auto_encode`):**
    *   Runs `ab-av1.exe` via `subprocess.Popen`.
    *   Sends PID back via `pid_callback`.
    *   Reads stdout/stderr line by line.
    *   Calls `_update_stats_from_line` for each line.
5.  **Parsing & Callbacks (`ab_av1_wrapper._update_stats_from_line`):**
