...
from .parser import AbAv1Parser # Keep using the parser
...
class AbAv1Wrapper:
    ...
    # Removed _log_consolidated_progress
    ...
    # Removed _find_ffmpeg_log function entirely
    ...
    def auto_encode(self, input_path: str, output_path: str,
        ...
        while current_vmaf_target >= MIN_VMAF_FALLBACK_TARGET:
            ...
            cmd_for_log = [ os.path.basename(self.executable_path), "auto-encode", "-i", os.path.basename(anonymized_input_path), "-o", os.path.basename(anonymized_output_path), "--preset", str(preset), "--min-vmaf", str(current_vmaf_target) ]
            ...
            if self.file_info_callback:
                 callback_info = {
                    "message": "", "original_vmaf": initial_min_vmaf,
                    "fallback_vmaf": current_vmaf_target, "used_fallback": current_vmaf_target != initial_min_vmaf,
                    "vmaf_target_used": current_vmaf_target, "original_size": stats.get("original_size")
                 }
            ...
            try:
                ...
                # Revert to the working approach: redirect stderr to stdout
                process = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                                          ...
                                          encoding='utf-8',
                                          errors='replace',
                                          env=process_env) # Use potentially modified env

                # Fix missing callback
                if pid_callback: pid_callback(process.pid)
                
                # Use a simpler reading loop with no multithreading
                import threading
                ...
                try:
                    # Simple blocking readline loop - single-threaded, easier to debug
                    for line in iter(process.stdout.readline, ''):
                        line = line.strip()
                        if not line:
                            continue
                            
                        # Special handling for anything that might be progress info
                        if '%' in line:
                ...
                if read_loop_exception:
                    # It's usually better to raise errors after the process has exited
                    raise AbAv1Error(f"Exception during pipe read: {read_loop_exception}", command=cmd_str, output=full_output_text, error_type="pipe_read_error") from read_loop_exception


            except FileNotFoundError as e:
                error_msg = f"Executable not found: {self.executable_path} - {e}"; logger.error(error_msg)
                ...
                error_msg = f"Failed to start or manage process during VMAF {current_vmaf_target}: {str(e)}"; logger.error(error_msg, exc_info=True)
                ...
                if process and process.poll() is None:
                    try: logger.warning("Terminating/Killing runaway process due to error."); process.terminate(); time.sleep(0.2); process.kill()
                    except: pass
                ...
                if not isinstance(e, (AbAv1Error, FileNotFoundError)): # Avoid re-wrapping known types
                    raise AbAv1Error(error_msg, command=cmd_str, output=full_output_text, error_type="process_management_failed") from e
                else:
            ...
            if return_code == 0:
                if not os.path.exists(output_path):
                     ...
                     last_error_info = {"return_code": return_code, "error_type": "missing_output_on_success", "error_details": error_msg, "command": cmd_str_log, "output_tail": full_output_text.splitlines()[-20:]}
                else:
                    success = True
                    logger.info(f"Encode succeeded (rc=0) for {anonymized_input_path} with VMAF target {current_vmaf_target}")
                    break
            else:
                ...
                crf_fail_match = re.search(r'Failed\s+to\s+find\s+a\s+suitable\s+crf', full_output_text, re.IGNORECASE)
                if crf_fail_match:
                ...
                elif re.search(r'ffmpeg.*?:\s*Invalid\s+data\s+found', full_output_text, re.IGNORECASE): error_type="invalid_input_data"; error_details="Invalid data in input"
                ...
                last_error_info = {"return_code": return_code, "error_type": error_type, "error_details": error_details, "command": cmd_str_log, "output_tail": full_output_text.splitlines()[-20:] if full_output_text else ["<No output captured>"]}
                logger.warning(f"[Attempt VMAF {current_vmaf_target}] Failed (rc={return_code}): {error_details} (Type: {error_type})")
        ...
        if not success:
            ...
            if last_error_info:
                ...
                exc_map = { "invalid_input_data": InputFileError, "file_not_found": InputFileError, "file_open_failed": InputFileError, "no_video_stream": InputFileError, "analysis_failed": InputFileError, "vmaf_calculation_failed": VMAFError, "encoding_failed": EncodingError, "memory_error": EncodingError, "crf_search_failed": EncodingError, "executable_not_found": EncodingError, "permission_denied": EncodingError, "output_dir_creation_failed": OutputFileError, "rename_failed": OutputFileError, "missing_output_on_success": OutputFileError, "pipe_read_error": AbAv1Error }
                ...
                generic_error_msg = f"Encode failed for {anonymized_input_path} for unknown reasons after loop."; logger.error(generic_error_msg)
        ...
        else: logger.warning("Final size reduction could not be determined from parsing.")

        # --- Post-Success Sanity Checks ---
        if not os.path.exists(output_path):
             error_msg = f"CRITICAL: Output file missing after reported success: {anonymized_output_path}"
             logger.error(error_msg)
             if self.file_info_callback: self.file_info_callback(os.path.basename(input_path), "failed", {"message":error_msg, "type":"missing_output_post_success"})
        ...
        if cleaned_count > 0: logger.info(f"Cleaned {cleaned_count} leftover temporary folder(s) in {cleanup_dir}.")
        else: logger.debug(f"No leftover temporary folders found to clean in {cleanup_dir}.")
