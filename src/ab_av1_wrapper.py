...
def init(self, message, command=None, output=None, error_type=None):
...
def _update_stats_from_line(self, line: str, stats: dict) -> None:
    ...
    try:
        ...
        # --- ENCODING PHASE ---
            ...
            # Check for FFMPEG progress percentage update (often includes other info)
            # Example: ' 17.5 %, 130.5 fps,   0 MB,   1 MB/s,  242 seconds left'
            # Make regex more robust to variations in spacing and content
            try:
                ...
                if progress_match:
                    ...
                    processed_line = True
                    # Actual logging/callback handled by the main loop reading FFMPEG lines

                    # Always include all available stats in callback
                    callback_data = {
            ...
            # Check for size information (might be separate line from ab-av1 itself)
            # Example: '      Output size:   10.98 MB (6.53% of source)'
            try:
                # Use \s+ for flexible spacing, allow for different units potentially? (though MB seems common)
                ...
                if size_match:
                    ...
                    processed_line = True
                    # Logged by the main loop's consolidated message
...
def auto_encode(self, input_path: str, output_path: str,
    ...
    while current_vmaf_target >= MIN_VMAF_FALLBACK_TARGET:
        ...
        cmd = [
            ...
            # Add other parameters here if needed (e.g., --pix-format, --enc-params from future settings)
        ]
        cmd_str = " ".join(cmd)
        ...
        cmd_for_log = [ # Anonymized log version
            ...
            # Add other parameters here for logging if needed
        ]
        ...
        if file_info_callback and current_vmaf_target != initial_min_vmaf:
             file_info_callback(os.path.basename(input_path), "retrying", {
                 ...
                 "used_fallback": True # Add flag indicating fallback was used
             })
        elif file_info_callback and 'original_size' in stats: # Check if stats has original size (first attempt)
        ...
        stats["last_ffmpeg_fps"] = None # Reset FPS/ETA for new attempt
        stats["eta_text"] = None
        ...
        try:
            for line in iter(process.stdout.readline, ""):
                ...
                if stats.get("phase") == "encoding" and stripped_line and "%" in stripped_line:
                    logger.info(f"[FFMPEG] {stripped_line}") # Keep logging raw ffmpeg lines

                    # --- Extract FFMPEG specific stats ---
                    try:
                        # Extract fps if present in the line (use \s+)
                        fps_match = re.search(r'(\d+\.?\d*)\s+fps', stripped_line)
                        if fps_match:
                            stats["last_ffmpeg_fps"] = fps_match.group(1)
                    except (AttributeError, ValueError, IndexError) as e:
                         logger.warning(f"Could not parse FPS from ffmpeg line: '{stripped_line}' - {e}")

                    try:
                        # Extract eta if present (handle various formats: seconds, min:sec, h:min:sec)
                        ...
                        eta_match_time = re.search(r'eta\s+(\d+:\d{2}:\d{2})\b', stripped_line, re.IGNORECASE) # H:MM:SS
                        eta_match_min_sec = re.search(r'eta\s+(\d+:\d{2})\b', stripped_line, re.IGNORECASE) # MM:SS

                        if eta_match_time: stats["eta_text"] = f"{eta_match_time.group(1)}"
                        elif eta_match_min_sec: stats["eta_text"] = f"0:{eta_match_min_sec.group(1)}" # Add hour for consistency
                        ...
                        logger.warning(f"Could not parse ETA from ffmpeg line: '{stripped_line}' - {e}")
                    # --- End FFMPEG stat extraction ---
                    ...
                    if os.path.exists(temp_output_path):
                        ...
                        current_encoding_progress = stats.get("progress_encoding", 0) # Get progress parsed by _update_stats_from_line
                        if current_temp_size > 0 and current_encoding_progress > 0:
                            try:
                                # Estimate final size based on current progress
                                estimated_final_size = current_temp_size / (current_encoding_progress / 100.0)
                                stats["estimated_output_size"] = estimated_final_size # Store estimate
                                ...
                                if self.file_info_callback and stats.get("original_size") and estimated_final_size > 0:
                                    ...
                                    self.file_info_callback(os.path.basename(stats.get("input_path", "unknown")), "progress", {
                                        "phase": "encoding",
                                        "progress_encoding": current_encoding_progress,
                                        "output_size": estimated_final_size,
                                        "original_size": original_size,
                                        "size_reduction": reduction_percent,
                                        "is_estimate": True # Mark this size as an estimate
                                    })
                            except ZeroDivisionError:
                                logger.debug("Division by zero calculating estimated size (progress likely 0).")
                            except Exception as e:
                                logger.error(f"Error calculating estimated size: {e}")


                    # Log consolidated progress message including FFMPEG info
                ...
                # Update statistics based on this line (handles VMAF/CRF/progress %/etc)
        ...
        # --- Error Handling for this attempt ---
            ...
            # Use \s+ and make case-insensitive for robustness
            if re.search(r'ffmpeg.*?:\s*Invalid\s+data\s+found', full_output_text, re.IGNORECASE): error_type="invalid_input_data"; error_details="Invalid data in input"
                ...
                error_type = "crf_search_failed"; error_details = f"Could not find suitable CRF for VMAF {current_vmaf_target}"
                 ...
                 error_type = "executable_not_found"; error_details = "ab-av1.exe command failed (not found or path issue?)"
            # Add more specific error checks based on observed failures if needed
    ...
    # Ensure final parsing happens *before* reporting completion
    self._parse_final_output(full_output_text, stats) # This acts as fallback parser
    ...
    try:
        ...
        # Ensure temp folders are cleaned even if rename fails
    ...
    # Send completion update - make sure stats are final
    if file_info_callback:
        final_stats_for_callback = {
            ...
            "size_reduction": stats.get('size_reduction'), # Include final size reduction
            "output_path": output_path # Include final output path
            }
        # Get final output size if possible
        try:
    ...
    # Clean temp folders on success too
    cleaned_count = clean_ab_av1_temp_folders(output_dir);
...
def _parse_final_output(self, output_text: str, stats: dict) -> None:
    ...
    try:
        # Look for the 'Output size: ... (...% of source)' pattern
        ...
        if size_percent_matches:
            ...
            if abs(stats.get("size_reduction", -1) - final_reduction) > 0.01:
            ...
            # Try calculating from absolute sizes if percentage not found/parsed
            ...
            output_size_match = re.search(r'Output\s+size:\s+(\d+\.?\d*)\s+(\w+)', output_text, re.IGNORECASE) # Match without percentage part
