...
def handle_progress(gui, filename: str, info: dict) -> None:
    ...
    update_ui_safely(
        gui.root, update_progress_bars, gui, progress_event.progress_quality, progress_event.progress_encoding
    )

    # Update current file message
    update_ui_safely(
        gui.root,
        lambda f=filename, m=progress_event.message: gui.current_file_label.config(text=f"Processing: {f} - {m}"),
    )
