...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        logger.warning("Attempted run with command line arguments (not supported). Exiting.")
        sys.exit(1)
    ...
    try:
        ...
        logger.critical(f"An unhandled error occurred during application startup or runtime: {e}", exc_info=True)
        # Attempt cleanup if app object exists
        if app and hasattr(app, "on_exit"):
            try:
                logger.info("Attempting application cleanup via on_exit due to error...")
                app.on_exit()
            except Exception as cleanup_e:
                logger.error(f"Error during on_exit cleanup: {cleanup_e}", exc_info=True)
        elif root:
            try:
