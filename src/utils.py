...
def get_video_info(video_path: str) -> dict:
    ...
    try:
        ...
        return info
    except subprocess.CalledProcessError as e: logging.exception(f"ffprobe failed for {anonymize_filename(video_path)}: {e.stderr}"); return None
    except json.JSONDecodeError as e: logging.exception(f"ffprobe JSON error for {anonymize_filename(video_path)}: {e}"); return None
    except FileNotFoundError: logging.exception("ffprobe not found."); return None
    except Exception as e: logging.exception(f"ffprobe unexpected error {anonymize_filename(video_path)}: {e}"); return None
        ...
        try:
            ...
            def _safe_update_wrapper():
                try:
                    ...
                    logging.exception(f"Error in UI update function {update_function.__name__ if hasattr(update_function, '__name__') else 'lambda'}: {inner_e}")

            # Schedule on the main thread
            root.after(0, _safe_update_wrapper)
        except Exception as e:
            logging.exception(f"Error scheduling UI update for {update_function.__name__ if hasattr(update_function, '__name__') else 'lambda'}: {e}")
    else:
        logging.debug("Skipped UI update: root widget invalid or destroyed")
...
def log_conversion_result(input_path: str, output_path: str, elapsed_time: float) -> None:
    ...
    try:
        ...
        logging.exception(f"Error logging conversion result for {anonymize_filename(output_path)}: {e}")
...
def load_history() -> list:
    ...
    if os.path.exists(history_path):
        try:
            ...
            logging.exception(f"Error loading history {history_path}: {e}")
            return []
    return []
def append_to_history(record_dict: dict) -> None:
    ...
    try:
        ...
        logging.exception(f"Error saving history {history_path}: {e}")
    except Exception as e:
...
def prevent_sleep_mode() -> bool:
    ...
    try:
        ...
        logging.exception(f"Failed to prevent system sleep: {e}")
        return False
...
def allow_sleep_mode() -> bool:
    ...
    try:
        ...
        logging.exception(f"Failed to restore normal power management: {e}")
        return False
...
def estimate_remaining_time(gui, current_file_info: dict = None) -> float:
    ...
    logging.debug("Total remaining ETA calculation:")
